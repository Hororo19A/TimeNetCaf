...
HOURLY_RATE      = 0.1          # PHP per hour
MINUTE_RATE      = HOURLY_RATE / 60
ADMIN_EXIT_PIN   = "1234"       # PIN to exit kiosk mode
PAYMONGO_MIN_PHP = 1.00         # minimum charge for online payments
...
def entry_field(parent, label, var, secret=False, width=32):
    tk.Label(parent, text=label, fg=TEXT3, bg=BG2,
             font=(FB, 8, "bold")).pack(anchor="w", pady=(0, 3))
    frame = tk.Frame(parent, bg=BG4, highlightthickness=1,
                     highlightbackground=BORDER2)
    frame.pack(fill="x", pady=(0, 16))
    e = tk.Entry(frame, textvariable=var,
                 show=("●" if secret else ""),
                 bg=BG4, fg=TEXT, insertbackground=AMBER,
                 font=(FB, 12), relief="flat", width=width)
    e.pack(padx=14, pady=10, fill="x")
    e.bind("<FocusIn>",  lambda _: frame.config(highlightbackground=AMBER))
    e.bind("<FocusOut>", lambda _: frame.config(highlightbackground=BORDER2))
    return e, frame

def pill_button(parent, text, command, bg=AMBER, fg=BG, width=None, pady=14):
    kw = {}
    if width:
        kw["width"] = width
    return tk.Button(parent, text=text, command=command,
                     ...
                     activebackground=AMBER2 if bg == AMBER else bg,
                     activeforeground=fg,
...
class ThemedDialog(tk.Toplevel):
    ...
    def _build(self, color, dim, icon, title, message, detail):
        ...
        if message:
            tk.Label(self, text=message, bg=BG2, fg=TEXT2,
...
# ════════════════════════════════════════════════════════════════════
#  ADMIN PIN DIALOG  (Ctrl+Shift+A to exit kiosk)
# ════════════════════════════════════════════════════════════════════
...
class AdminPinDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(self, text="Admin Access Required", bg=BG2, fg=TEXT,
                 font=(FD, 15, "bold")).pack(pady=(6, 2))
        tk.Label(self, text="Enter PIN to exit kiosk mode",
...
class CashVoucherDialog(tk.Toplevel):
    """
    Shown to the customer after they choose cash payment.
    Polls the DB every second until the admin activates the voucher.
    """
    ...
    def _build(self):
        c = tk.Canvas(self, bg=BG2, height=4, highlightthickness=0)
        c.pack(fill="x")
        c.bind("<Configure>", lambda e: gradient_bar(c, 0, 0, e.width, 4, GREEN, CYAN))
        tk.Label(self, text="💵", bg=BG2, fg=GREEN, font=(FD, 44)).pack(pady=(28, 0))
        tk.Label(self, text="Cash Payment", bg=BG2, fg=TEXT,
                 font=(FD, 17, "bold")).pack(pady=(6, 2))
        tk.Label(self, text="Show voucher code at the counter",
                 bg=BG2, fg=TEXT2, font=(FB, 10)).pack(pady=(0, 16))
        hsep(self).pack(fill="x", padx=24)

        # Session summary card
        s = tk.Frame(self, bg=BG3, highlightthickness=1, highlightbackground=BORDER2)
        ...
        for lbl, val, col in [
            ...
            tk.Label(r, text=val, fg=col, bg=BG3, font=(FD, sz, wt)).pack(side="right")
        tk.Frame(s, bg=BG3, height=6).pack()

        hsep(self).pack(fill="x", padx=24)

        # Voucher code card
        vc = tk.Frame(self, bg=BG3, highlightthickness=2, highlightbackground=GREEN)
        vc.pack(padx=28, pady=18, fill="x")
        tk.Label(vc, text="VOUCHER CODE", fg=TEXT3, bg=BG3,
                 font=(FB, 8, "bold")).pack(pady=(14, 6))
        tk.Label(vc, text=self.voucher_code, fg=GREEN, bg=BG3,
                 font=(FM, 26, "bold")).pack(pady=(0, 6))

        def _copy():
            self.app.clipboard_clear()
            self.app.clipboard_append(self.voucher_code)
            cb.config(text="✓  Copied!", bg=GREEN_DIM, fg=GREEN)
            self.after(2200, lambda: cb.config(text="📋  Copy Code", bg=BG5, fg=CYAN))
        ...
        sf.pack(pady=(14, 0))
        self.status_lbl = tk.Label(sf, text="⏳  Waiting for staff to activate…",
                                   bg=BG2, fg=YELLOW, font=(FB, 11, "bold"))
        self.status_lbl.pack()
        self.dot_lbl = tk.Label(sf, text="●○○", bg=BG2, fg=TEXT3, font=(FB, 10))
        self.dot_lbl.pack(pady=(4, 0))
        tk.Label(self, text="Pay the cashier and give them your voucher code.\n"
                            "Your PC will start automatically once confirmed.",
                 bg=BG2, fg=TEXT2, font=(FB, 9), justify="center").pack(pady=(8, 0))

        hsep(self).pack(fill="x", padx=24, pady=16)
...
class PaymentWaitingScreen:
    """
    Launches a kiosk-mode browser for the PayMongo checkout URL,
    overlays a top bar with status, and polls PayMongo every 3 s
    (via the background PayMongoAPI thread) until payment is confirmed.
    Falls back to an overlay dialog if no browser is found.
    """
    BAR_H = 72
    ...
    def _build_top_bar(self):
        ...
        gc = tk.Canvas(bar, bg=BG2, height=3, highlightthickness=0)
        gc.pack(fill="x")
        gc.bind("<Configure>", lambda e: gradient_bar(gc, 0, 0, e.width, 3, color, AMBER))
        ...
        self._dot_lbl = tk.Label(inner, text="●○○", bg=BG2, fg=TEXT3, font=(FB, 10))
        ...
        tk.Button(inner, text="✕  Cancel Payment", command=self._cancel,
                  ...
                  highlightthickness=1, highlightbackground=RED).pack(
            side="right", pady=12)
        self._animate()

    # ── fallback overlay (no browser found) ───────────────────────
    ...
    def _build_overlay(self):
        ...
        self._dot_lbl = tk.Label(centre, text="Checking payment status  ●○○",
    ...
    def _start_watcher(self):
        def _w():
            while not self._done:
                time.sleep(0.05)
                # Payment confirmed when PayMongoAPI removes the link_id from pending
                if self.link_id not in paymongo.pending:
                    if not self._done:
                        self.app.after(0, self._on_confirmed)
                    return
                # If the user closed the browser manually → cancel
                proc = self._browser
...
class PayMongoAPI:
    ...
    def create_link(self, amount, desc, info):
        ...
        try:
            r = requests.post(f"{PAYMONGO_BASE_URL}/links",
                              headers=paymongo_headers(), json=payload, timeout=20)
            d = r.json()
            if r.status_code not in (200, 201):
                errs = d.get("errors", [])
                msg  = errs[0].get("detail", "Error") if errs else f"HTTP {r.status_code}"
    ...
    def activate(self, info):
        """Write the active session + payment record to the DB."""
        sid = info["session_id"]
        ...
        db_exec(
            ...
            (sid, cid))
        # Insert session row if it somehow doesn't exist yet
        existing = db_exec("SELECT id FROM sessions WHERE id=%s", (sid,), fetch=True)
        if not existing:
            db_exec("""
                INSERT IGNORE INTO sessions
                  (id, user_id, computer_id, duration, cost, status, start_time, created_at)
    ...
    def _loop(self):
        """Background thread: polls every 3 s for each pending payment link."""
        while True:
            time.sleep(3)
...
class App(tk.Tk):
    ...
    # ── payment success callback (runs on main thread via after()) ─

    def _on_payment_success(self, info):
        # ── FIX: schedule on main thread, give DB a moment to commit ──
    ...
    def _set_taskbar(visible):
        ...
        try:
            ...
            for cls in ("Shell_TrayWnd", "Shell_SecondaryTrayWnd"):
                hw = user32.FindWindowW(cls, None)
                if hw:
    ...
    def unlock_for_session(self):
        """Shrink window to a small floating timer after session starts."""
        self._locked = False
        self._taskbar_enforce_on = False
        self.overrideredirect(False)
        self.update_idletasks()
    ...
    def _setup_ttk(self):
        ...
        s.configure("Session.Horizontal.TProgressbar",
                    troughcolor=BG3, background=AMBER, thickness=8, bordercolor=BG3)
...
class LoginPage(tk.Frame):
    ...
    def _build(self):
        ...
        gc.pack(fill="x")
        gc.bind("<Configure>", lambda e: gradient_bar(gc, 0, 0, e.width, 4, AMBER, CYAN))

        tk.Label(card, text="🖥", bg=BG2, fg=AMBER, font=(FD, 36)).pack(pady=(30, 0))
        ...
        eu, _ = entry_field(form, "USERNAME", self.usr)
        ep, _ = entry_field(form, "PASSWORD", self.pwd, secret=True)
        ep.bind("<Return>", lambda _: self._login())
        pill_button(form, "Sign In →", self._login).pack(fill="x", pady=(0, 6))
        ...
        rl = tk.Label(footer, text="Create an account", fg=AMBER, bg=BG2,
    ...
    def _login(self):
        ...
        if not u or not p:
            ...
            return
        rows = db_exec(
            "SELECT * FROM users WHERE username=%s AND password=%s", (u, p), fetch=True)
        if rows:
            user = dict(rows[0])
            if user.get("role") == "admin":
                self.err.config(text="✗  Use the Admin portal for admin accounts.")
                return
            Auth.login(user)
...
class RegisterPage(tk.Frame):
    ...
    def _build(self):
        ...
        gc = tk.Canvas(card, bg=BG2, height=4, highlightthickness=0, width=440)
        gc.pack(fill="x")
        gc.bind("<Configure>", lambda e: gradient_bar(gc, 0, 0, e.width, 4, PURPLE, CYAN))

        tk.Label(card, text="✨", bg=BG2, fg=PURPLE, font=(FD, 30)).pack(pady=(28, 0))
        ...
        self.fields = {k: tk.StringVar() for k in ("username", "password", "confirm")}
        eu, _ = entry_field(form, "USERNAME",         self.fields["username"])
        ep, _ = entry_field(form, "PASSWORD",         self.fields["password"], secret=True)
        ec, _ = entry_field(form, "CONFIRM PASSWORD", self.fields["confirm"],  secret=True)
        ec.bind("<Return>", lambda _: self._register())
        tk.Button(form, text="Create Account →", command=self._register,
                  ...
                  padx=20, pady=14).pack(fill="x", pady=(8, 6))
        ...
        tk.Label(footer, text="Already have an account? ", fg=TEXT2, bg=BG2,
                 font=(FB, 10)).pack(side="left")
        bl = tk.Label(footer, text="Sign in here", fg=AMBER, bg=BG2,
                      cursor="hand2", font=(FB, 10, "bold"))
        bl.pack(side="left")
    ...
    def _register(self):
        ...
        if len(d["password"]) < 6:
            ...
            return
        exists = db_exec(
            "SELECT id FROM users WHERE username=%s", (d["username"],), fetch=True)
        if exists:
            self.err.config(text="✗  Username already taken.")
            return
        new = {
            "id":       f"user-{now_ms()}",
            "username": d["username"],
        ...
        db_exec("""
            ...
            VALUES (%s, %s, %s, %s, %s)
        """, (new["id"], new["username"], new["password"], new["role"], now_ms()))
        Auth.login(new)
        self.app.show_customer_dashboard()
...
class PaymentDialog(tk.Toplevel):
    ...
    def __init__(self, parent_dash, computer_name, duration, cost, on_complete):
        ...
        self.update_idletasks()
        w  = 520
        h  = max(self.winfo_reqheight() + 40, 640)
        sw = self.winfo_screenwidth()
        sh = self.winfo_screenheight()
        self.geometry(f"{w}x{h}+{(sw-w)//2}+{(sh-h)//2}")
    ...
    def _build(self):
        ...
        gc.pack(fill="x")
        gc.bind("<Configure>", lambda e: gradient_bar(gc, 0, 0, e.width, 4, AMBER, CYAN))
        tk.Label(self, text="💳  Complete Payment", bg=BG2, fg=TEXT,
                 font=(FD, 16, "bold")).pack(pady=(22, 2))
        tk.Label(self, text="Choose how you'd like to pay",
                 bg=BG2, fg=TEXT2, font=(FB, 10)).pack(pady=(0, 18))
        hsep(self).pack(fill="x", padx=24)

        # Session summary
        sc = tk.Frame(self, bg=BG3, highlightthickness=1, highlightbackground=BORDER2)
        ...
        for lbl, val, col in [
            ...
            ("Total Due", fmt_currency(self.cost),      AMBER),
        ]:
        ...
        hsep(self).pack(fill="x", padx=24)
        tk.Label(self, text="PAYMENT METHOD", fg=TEXT3, bg=BG2,
                 font=(FB, 8, "bold")).pack(pady=(18, 10))

        # Method selector cards
        self._m_widgets = {}
        mf = tk.Frame(self, bg=BG2)
        ...
        for ci, (val, icon, lbl_txt, color) in enumerate(self.METHODS):
            ...
            outer  = tk.Frame(mf, bg=color if is_sel else BORDER, cursor="hand2")
            outer.grid(row=0, column=ci, padx=5, pady=4, sticky="nsew")
            inner  = tk.Frame(outer, bg=BG4 if is_sel else BG3)
            inner.pack(fill="both", expand=True, padx=2, pady=2)
            il = tk.Label(inner, text=icon, bg=inner["bg"], fg=color, font=(FD, 22))
            il.pack(pady=(14, 2))
            nl = tk.Label(inner, text=lbl_txt, bg=inner["bg"], fg=TEXT, font=(FB, 11, "bold"))
            nl.pack(pady=(0, 14))
            self._m_widgets[val] = {"outer": outer, "inner": inner,
                                    "il": il, "nl": nl, "color": color}
            for w in (outer, inner, il, nl):
                w.bind("<Button-1>", lambda e, v=val: self._pick(v))

        self.note_lbl = tk.Label(self, text="", fg=TEXT2, bg=BG2,
                                 wraplength=460, font=(FB, 10), justify="center")
        self.note_lbl.pack(pady=(14, 0), padx=28)
        self._update_note()

        hsep(self).pack(fill="x", padx=24, pady=18)
        bf = tk.Frame(self, bg=BG2)
        bf.pack(padx=28, pady=(0, 32), fill="x")
        bf.columnconfigure(0, weight=1)
        bf.columnconfigure(1, weight=1)
        ghost_button(bf, "Cancel", self.destroy, TEXT2, 18).grid(
            row=0, column=0, sticky="nsew", padx=(0, 6))
        self.confirm_btn = tk.Button(
            bf, text=f"Pay {fmt_currency(self.cost)}  →",
            command=self._confirm,
            bg=AMBER, fg=BG, font=(FD, 13, "bold"),
            relief="flat", cursor="hand2",
            activebackground=AMBER2, activeforeground=BG,
            padx=16, pady=18)
    ...
    def _handle_cash(self):
        ...
        db_exec("""
            INSERT INTO sessions
              (id, user_id, computer_id, duration, cost, status, voucher_code, created_at)
        ...
        CashVoucherDialog(self.app, session_id, voucher, computer["name"],
                          self.duration, self.cost, _activated, _cancelled)
    ...
    def _handle_paymongo(self, method):
        if not Auth.user:
            ...
            return
        self.confirm_btn.config(state="disabled", text="⏳  Creating link…")
        self.update()
        session_id    = f"sess-{now_ms()}"
        ...
        desc = f"TimeNet Cafe — {computer_name} — {self.duration} min ({method.upper()})"
    ...
    def _on_link_result(self, result, method):
        ...
        def _paid():
            # ── FIX: directly force-load the active session, bypassing
            #    the polling guard that would skip a rebuild ──────────
            if isinstance(self.app.current_frame, CustomerDashboard):
                self.app.current_frame._force_load_session()
            if self.on_complete:
                self.on_complete()
        ...
        PaymentWaitingScreen(self.app, lid, url, method, self.cost, _paid, _cancelled)
...
class CustomerDashboard(tk.Frame):
    ...
    def _build(self):
        ...
        gc = tk.Canvas(self.hdr, bg=BG2, height=3, highlightthickness=0)
        gc.pack(fill="x")
        gc.bind("<Configure>", lambda e: gradient_bar(gc, 0, 0, e.width, 3, AMBER, CYAN))
        hi = tk.Frame(self.hdr, bg=BG2)
        hi.pack(fill="x", padx=28, pady=14)
        left = tk.Frame(hi, bg=BG2)
        left.pack(side="left")
        tk.Label(left, text="🖥", bg=BG2, fg=AMBER, font=(FD, 18)).pack(
            side="left", padx=(0, 10))
        tc = tk.Frame(left, bg=BG2)
        tc.pack(side="left")
        tk.Label(tc, text="TimeNet Cafe", bg=BG2, fg=TEXT,
                 font=(FD, 14, "bold")).pack(anchor="w")
        self.welcome_lbl = tk.Label(tc, text="", bg=BG2, fg=TEXT2, font=(FB, 10))
    ...
    def _show_booking(self):
        ...
        if not available:
            ...
            tk.Label(c, text="All PCs are currently in use. Please check back shortly.",
        ...
        accent_bar(pc_card, AMBER).pack(fill="x")
        pi = tk.Frame(pc_card, bg=BG2)
        pi.pack(padx=28, pady=20, fill="x")
        tk.Label(pi, text="🖥", bg=BG2, fg=AMBER, font=(FD, 36)).pack(
            side="left", padx=(0, 20))
        info = tk.Frame(pi, bg=BG2)
        info.pack(side="left")
        tk.Label(info, text=self.assigned_computer["name"], bg=BG2, fg=TEXT,
                 font=(FD, 22, "bold")).pack(anchor="w")
        rf = tk.Frame(info, bg=BG2)
        rf.pack(anchor="w")
        tk.Label(rf, text=f"{fmt_currency(HOURLY_RATE)} / hour",
                 fg=AMBER, bg=BG2, font=(FD, 12, "bold")).pack(side="left")
        tk.Label(rf, text="  ●  Ready", fg=GREEN, bg=BG2, font=(FB, 10)).pack(side="left")

        # Duration picker
        picker = tk.Frame(col, bg=BG2, highlightthickness=1, highlightbackground=BORDER2)
        picker.pack(fill="x")
        ...
        for idx, tier in enumerate(PRICING_TIERS):
            ...
            rn, cn = divmod(idx, 4)
            f = tk.Frame(grid, bg=BG3, cursor="hand2", width=160, height=80,
    ...
    def _pick_tier(self, minutes):
        ...
        f.config(bg=AMBER_DIM, highlightbackground=AMBER)
        l1.config(bg=AMBER_DIM, fg=AMBER)
        l2.config(bg=AMBER_DIM, fg=TEXT)
        cost = calc_cost(minutes)
        self.pay_btn.config(
            text=f"Pay {fmt_currency(cost)} & Start Session  →",
            bg=AMBER, fg=BG, state="normal", activebackground=AMBER2)
    ...
    def _session_widget(self, computer):
        ...
        accent_bar(outer, AMBER, 2).pack(fill="x")
        tk.Label(outer, text=f"🖥  {computer['name']}",
                 fg=TEXT3, bg=BG2, font=(FB, 8, "bold")).pack(pady=(8, 0))
        self.time_lbl = tk.Label(outer, text="00:00:00",
                                 fg=TEXT, bg=BG2, font=(FD, 24, "bold"))
        self.time_lbl.pack(pady=(2, 2))
        ...
        self.pct_lbl = tk.Label(row, text="100%", fg=AMBER, bg=BG2,
                                font=(FB, 8, "bold"))
        self.pct_lbl.pack(side="left")
        total_min = self.active_session.get("duration", 60)
        start_ms  = self.active_session.get("startTime", now_ms())
        end_dt    = datetime.fromtimestamp((start_ms + total_min * 60 * 1000) / 1000)
        tk.Label(row, text=f"Ends {end_dt.strftime('%I:%M %p')}",
                 fg=YELLOW, bg=BG2, font=(FB, 8, "bold")).pack(side="right")
        # ── FIX: always start a fresh tick loop after building widget ──
        self._ticking = True
        self._tick()
    ...
    def _tick(self):
        ...
        try:
            ...
            self.time_lbl.config(fg=clr)
            self.pct_lbl.config(
                text=f"{pct:.0f}%",
                fg=RED if pct < 10 else YELLOW if pct < 20 else AMBER)
        except Exception:
            self._ticking = False
            return
    ...
    def _end_session(self):
        ...
        db_exec(
            "UPDATE computers SET status='available', current_session_id=NULL WHERE id=%s",
    ...
    def _force_load_session(self):
        """
        Called after payment is confirmed (GCash/Maya/Cash).
        Always rebuilds the session view for the current user's active session,
        even if self.active_session was already set (to avoid the _ticking guard
        in _load_data skipping the widget rebuild).
        """
        if not Auth.user:
            return
        rows = db_exec(
        ...
        if not rows:
            # DB write may not have flushed yet — retry once after a short delay
            self.after(400, self._force_load_session)
            return
        # Reset ticking so _session_widget starts a fresh loop
        self._ticking = False
    ...
    def _load_data(self):
        """
        Called by the 2-second poll loop and on page transitions.
        Skips rebuilding the UI if the same session is already active
        (avoids interrupting the running _tick loop).
        """
        if not Auth.user:
            return
        rows = db_exec(
        ...
        if active:
            # ── FIX: only rebuild the session widget when the session ID
            #    changes.  If it's the same session already ticking, do
            #    nothing — the running _tick() loop is still correct. ──
            if (self.active_session
                    and self.active_session["id"] == active["id"]
                    and self._ticking):
                return
            self._ticking = False          # stop any stale tick loop
            self.active_session = active
            self._show_active_session()
            return

        # No active session — show booking screen
        try:
            self.hdr.pack(fill="x", before=self.content)
        except Exception:
            self.hdr.pack(fill="x")
        ...
        self.welcome_lbl.config(
            text=f"Welcome back, {Auth.user['username']}" if Auth.user else "")
        computers = db_exec("SELECT * FROM computers", fetch=True) or []
        self.assigned_computer = next(
            (c for c in computers if c["status"] == "available"), None)
        self._show_booking()

    def _poll(self):
        """Refresh from DB every 2 seconds for live sync."""
        self._load_data()
